...
class ZohoCRMService:
    ...
    def _refresh_token_if_needed(self, response):
        """
        Check if the token has expired (i.e. response.status_code==401) and refresh if needed.
    ...
    def update_entry_status(self, entry_id, status):
        ...
        """
        url = f'https://www.zohoapis.eu/crm/v2/Probenehmer/{entry_id}'
        headers = {
            'Authorization': f'Bearer {self.access_token}',
            'Content-Type': 'application/json'
        }
    ...
    def fetch_zoho_record_by_email(self, email):
        ...
        try:
            ...
            print(f"Fetch failed after retry: {e}")
            return None
        
    @retry_on_failure(max_retries=3, retry_delay=2, backoff_factor=2)
    def update_mailsent_status(self, entry_id, mail_sent):
        """
        Update CRM entry mail sent status with retry mechanism.
        """
        url = f'https://www.zohoapis.eu/crm/v2/Probenehmer/{entry_id}'
        headers = {
            'Authorization': f'Bearer {self.access_token}',
    ...
    def update_followup(self, entry_id, follow_up_count):
        """
        Update CRM entry follow-up count with retry mechanism.
        """
        url = f'https://www.zohoapis.eu/crm/v2/Probenehmer/{entry_id}'
        headers = {
            'Authorization': f'Bearer {self.access_token}',
